...
class Adversary:
    ...
    def generate_adv_whitebox(self, input_path, target_path, eps, num_iter):
        ...
        noise = tf.get_variable("Noise", shape=x.shape, trainable=True, initializer=tf.random_normal_initializer(),
                                dtype=tf.float32)
...
def trainable_noise(x, noise):


    noisy_img = tf.add(noise, x, name="AddNoise")
