...
def evaluate_attack(architecture, label, eval_dataset, batch_size, step_size, max_iter, normalize_images, cross):
    ...
    substitute = load_model('blackbox', architecture=architecture)
    if cross:
        bb_architecture = 'resnet50' if architecture == 'senet50' else 'senet50'
        blackbox = load_model('blackbox', architecture=bb_architecture)
    else:
        blackbox = substitute
    ...
    with sess:
        ...
        analyze_statistics(df, val_it.n, DESTINATION_PATH, label)
