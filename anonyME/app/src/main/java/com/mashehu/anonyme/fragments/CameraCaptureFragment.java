...
public class CameraCaptureFragment extends Fragment implements View.OnLayoutChangeListener {
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        if (viewModel.isBulkCaptureMode()) {
            ...
            {
                ImageView lastCapturePreview = getView().findViewById(
                        R.id.last_capture_preview);

                // Display image thumbnail in view
                assert getContext() != null;
                GlideApp.with(getContext())
                        .load(imagePaths.get(imagePaths.size() - 1))
                        .galleryThumbnail()
                        .into(lastCapturePreview);

            }
            else
            {
                Log.d(TAG, "Bulk mode but image list is empty");
            }
        }
