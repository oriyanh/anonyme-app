...
from AdvBox.advbox.attacks.tf.tools import fgsm
...
def get_pic_from_png(pic_path):
	resized_img = np.zeros((160, 160, 3))
	img = misc.imread(os.path.expanduser(pic_path), mode='RGB')
	resized_img[..., 0] = np.array(Image.fromarray(img[..., 0]).resize((160, 160)))  # Red channel
	resized_img[..., 1] = np.array(Image.fromarray(img[..., 1]).resize((160, 160)))  # Green channel
	resized_img[..., 2] = np.array(Image.fromarray(img[..., 2]).resize((160, 160)))  # Blue channel
	return regularize_pic(resized_img)


# return regularize_pic(misc.imresize(imageView, (160, 160, 3)))
